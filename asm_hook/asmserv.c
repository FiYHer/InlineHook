...
//FPU寄存器数组
const unsigned char *cuszFPUName[9] = {
  "ST0","ST1","ST2","ST3","ST4","ST5","ST6","ST7","FPU" };

//MMX寄存器数组
const unsigned char *cuszMMXName[9] = {
  "MM0","MM1","MM2","MM3","MM4","MM5","MM6","MM7","MMX" };

//CR寄存器数组
const unsigned char *cuszCRName[9] = {
  "CR0","CR1","CR2","CR3","CR4","CR5","CR6","CR7","CRX" };

//DR寄存器数组
const unsigned char *cuszDRName[9] = {
...
/* List of available processor commands with decoding, types of parameters and
 ...
 /*【带解码的可用处理器命令列表、参数类型和其他有用的信息。最后一个元素的字段掩码为0。如果助记符开始
 使用与号（“&”）时，其助记符根据操作数进行不同的解码大小（16或32位）。如果助记符以美元（“$”开头，则此助记符
 取决于地址大小。分号（“：”）将16位形式与32位形式分开，星号（“*”）将由w（16）、d（32）或none（16/32）替换。
 性格。如果命令是C_mmx或C_now类型，或者如果类型包含C_expl（=0x01），反汇编程序必须指定内存操作数的显式大小。】*/
const t_cmddata cstCmdData[] = {
