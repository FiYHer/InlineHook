...
retrylongjump:
	...
	namelen = strlen(name);
	for (pd = cstCmdData; pd->ulMask != 0; pd++) {
		if (pd->uszName[0] == '&') {            // Mnemonic depends on operand size
			j = 1;
			datasize = 2;
			addrsize = 4;
			while (1) {                      // Try all mnemonics (separated by ':')
				for (i = 0; pd->uszName[j] != '\0' && pd->uszName[j] != ':'; j++) {
					if (pd->uszName[j] == '*') {
						if (name[i] == 'W') { datasize = 2; i++; }
						else if (name[i] == 'D') { datasize = 4; i++; }
						else if (g_nSizeSens == 0) datasize = 2;
						else datasize = 4;
					}
					else if (pd->uszName[j] == name[i]) i++;
					else break;
				};
				if (name[i] == '\0' && (pd->uszName[j] == '\0' || pd->uszName[j] == ':'))
					break;                       // Bingo!
				while (pd->uszName[j] != '\0' && pd->uszName[j] != ':')
					j++;
				if (pd->uszName[j] == ':') {
					j++; datasize = 4;
		...
		else if (pd->uszName[0] == '$') {       // Mnemonic depends on address size
			j = 1;
			datasize = 0;
			addrsize = 2;
			while (1) {                      // Try all mnemonics (separated by ':')
				for (i = 0; pd->uszName[j] != '\0' && pd->uszName[j] != ':'; j++) {
					if (pd->uszName[j] == '*') {
						...
						else if (g_nSizeSens == 0) addrsize = 2;
						else addrsize = 4;
					}
					else if (pd->uszName[j] == name[i]) i++;
					else break;
			...
			addrsize = 4;
			while (1) {
				while (pd->uszName[j] != ',' && pd->uszName[j] != '\0') j++;
				if (j - k == namelen && strnicmp(name, pd->uszName + k, namelen) == 0) break;
				k = j + 1; if (pd->uszName[j] == '\0') break;
				j = k;
			};
			if (k > j) continue;
		};
		...
		nameok++;
		if (pd->ucArg1 == NNN || pd->ucArg1 >= PSEUDOOP)
			minop = 0;
		else if (pd->ucArg2 == NNN || pd->ucArg2 >= PSEUDOOP) {
			if (minop > 1) minop = 1;
			if (maxop < 1) maxop = 1;
		}
		else if (pd->ucArg3 == NNN || pd->ucArg3 >= PSEUDOOP) {
			if (minop > 2) minop = 2;
			if (maxop < 2) maxop = 2;
		}
		else
		...
		if (pd->ucBits == FF) datasize = 2;      // Forced 16-bit size
		if (pd->ucBits == WW || pd->ucBits == WS || pd->ucBits == W3 || pd->ucBits == WP)
		...
		for (j = 0; j < 3; j++) {              // Up to 3 operands
			op = aop + j;
			if (j == 0) arg = pd->ucArg1;
			else if (j == 1) arg = pd->ucArg2;
			else arg = pd->ucArg3;
			...
			case MMB:                      // Two adjacent memory locations (BOUND)
				if (op->type != MRG) match |= MA_TYP;
				k = op->size; if (g_nIdeal == 0 && k > 1) k /= 2;
	...
	*(ulong *)tcode = pd->ulCode & pd->ulMask;
	memset(tmask, 0, sizeof(tmask));
	*(ulong *)tmask = pd->ulMask;
	i = pd->ucLen - 1;                         // Last byte of command itself
	...
	if (pd->ucBits == WW || pd->ucBits == WS || pd->ucBits == WP) {
		if (datasize == 0) {
	...
	if ((pd->ucType & C_TYPEMASK) == C_NOW) {
		immsize = 1;
		immediate = (pd->ulCode >> 16) & 0xFF;
	};
	...
	for (j = 0; j < 3; j++) {                // Up to 3 operands
		...
		case MFX:                        // Memory in ModRM byte (ext. FPU state)
			...
			if (op->base < 0 && op->index < 0) {
				...
				if (op->base < 8) {
					if (op->segment != SEG_UNDEF && op->segment != cstAddr32[op->base].nDefSeg)
						segment = op->segment;
					tcode[i + 1] |=
		...
		case IMU:                        // Immediate unsigned data (8 or 16/32)
		case VXD:                        // VxD service (32-bit only)
			if (datasize == 0 && pd->ucArg2 == NNN && (pd->ucBits == SS || pd->ucBits == WS))
				datasize = 4;
			if (datasize == 0) {
				strcpy(errtext, "Please specify operand size");
				goto error;
	...
	if (lock != 0) {                       // Lock prefix specified
		model->uszCode[j] = 0xF0;
		model->uszMask[j] = 0xFF; j++;
	};
	if (datasize == 2 && pd->ucBits != FF) {   // Data size prefix necessary
		model->uszCode[j] = 0x66;
		model->uszMask[j] = 0xFF; j++;
	};
	if (addrsize == 2) {                   // Address size prefix necessary
		model->uszCode[j] = 0x67;
		model->uszMask[j] = 0xFF; j++;
	};
	if (segment != SEG_UNDEF) {            // Segment prefix necessary
		if (segment == SEG_ES) model->uszCode[j] = 0x26;
		else if (segment == SEG_CS) model->uszCode[j] = 0x2E;
		else if (segment == SEG_SS) model->uszCode[j] = 0x36;
		else if (segment == SEG_DS) model->uszCode[j] = 0x3E;
		else if (segment == SEG_FS) model->uszCode[j] = 0x64;
		else if (segment == SEG_GS) model->uszCode[j] = 0x65;
	...
	model->nJmpSize = jmpsize;
	model->nJmpOffset = jmpoffset;
	model->nJmpPos = i + j;
	if (jmpsize != 0) {
		if (ip != 0) {
	...
	memcpy(model->uszCode + j, tcode, i);
	memcpy(model->uszMask + j, tmask, i);
	i += j;
	model->nLength = i;
